...
public class SoulEntityRenderer extends EntityRenderer<SoulEntity> {
	...
	public void render(SoulEntity entity, float yaw, float tickDelta, MatrixStack matrices,
		...
		ParticleEffect dust = new DustParticleEffect(new Vec3f(0.0f, 0.0f, 0.0f), 0.75f);

		World world = client.world;
		boolean allParticles = client.options.getParticles().getValue() == ParticlesMode.ALL;
		...
		for (int j = 0; j < Math.round(direction.length() * 4); j++) {
			...
			ClientParticles.spawnWithMaxAge(ParticleTypes.PORTAL, currentRenderPosition,
