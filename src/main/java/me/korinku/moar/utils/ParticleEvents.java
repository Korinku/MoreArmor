...
public class ParticleEvents {
	...
	public static final ParticleSystemController CONTROLLER = new ParticleSystemController(
			new Identifier(MoreArmor.MOD_ID, "particles"));
	...
	public static final ParticleSystem<BlockPos> UNLINK_WEAVER = CONTROLLER.register(BlockPos.class,
			(world, pos, pedestal) -> {
				ClientParticles.setParticleCount(20);
				ClientParticles.spawnLine(ParticleTypes.SMOKE, world, pos.add(0.5, 0.4, 0.5),
	...
	public static final ParticleSystem<Boolean> EXTRACTION_RITUAL_FINISHED = CONTROLLER.register(Boolean.class,
			(world, pos, successful) -> {
				Vec3d loc = pos.add(.5, .5, .5);
	...
	public static final ParticleSystem<Direction> PEDESTAL_REMOVED = CONTROLLER.register(Direction.class,
			(world, pos, direction) -> {
				final Vec3d loc = pos.add(.5, 0, .5);

				ClientParticles.setParticleCount(20);
				ClientParticles.spawnPrecise(ParticleTypes.SMOKE, world,
						loc.add(direction.getOffsetX() * .15, .35, direction.getOffsetZ() * .15),
						direction.getOffsetZ() / 4d, 0.1, direction.getOffsetX() / 4d);
			});

	public static final ParticleSystem<BlockPos> LINK_SOUL_FUNNEL = CONTROLLER.register(BlockPos.class,
			(world, pos, offset) -> {
				...
				ClientParticles.setParticleCount(20);
				ClientParticles.spawnPrecise(ParticleTypes.WITCH, world, new Vec3d(offsetX, offsetY, offsetZ).add(pos),
						offset.getZ() / 12d, 0.1f, offset.getX() / 12d);
			});

	public static final ParticleSystem<Void> SOULFIRE_FORGE_SOULS = CONTROLLER.register(Void.class,
			(world, pos, data) -> {
				ClientParticles.setParticleCount(3);
